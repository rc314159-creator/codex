...
impl ThreadRequestProcessor {
    ...
    async fn read_thread_view(
        ...
        let has_live_in_progress_turn = if let Some(loaded_thread) = loaded_thread.as_ref() {
            matches!(loaded_thread.agent_status().await, AgentStatus::Running)
                || self
                    .live_active_turn_snapshot(thread_id)
                    .await
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            ...
            let history = loaded_thread
                ...
                .await
                .map_err(|err| thread_read_history_load_error(thread_id, err))?;
            thread.turns = reconstruct_thread_turns_for_thread_read(
                &history.items,
                self.live_active_turn_snapshot(thread_id).await,
            );
        }
    ...
    /// `thread/read` must expose the same live turn snapshot as `thread/turns/list`.
    /// Persisted rollout history can lag while the current turn is still running.
    async fn merge_active_turn_into_thread_read(&self, thread_id: ThreadId, thread: &mut Thread) {
        let active_turn = self.live_active_turn_snapshot(thread_id).await;
...
fn reconstruct_thread_turns_for_thread_read(
    items: &[RolloutItem],
    active_turn: Option<Turn>,
) -> Vec<Turn> {
    let mut turns = build_legacy_api_turns_from_rollout_items(items);
